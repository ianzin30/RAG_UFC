...
class RAGServiceQuestionAnsweringMixin(RAGServiceFollowUpMixin):
    ...
    def _extract_company_like_candidates(self, text: str) -> list[str]:
        ...
        patterns = (
            ...
            r"(?:conv[eê]nio\s+com(?:\s+a)?\s+empresa\s+)([A-Z][A-Za-z0-9&./-]*(?:\s+[A-Z][A-Za-z0-9&./-]*){0,3})",
    ...
    def _infer_short_answer_shape(self, resolved_question: str) -> str:
        ...
        phrase_markers = {
            "money": {"orcamento total", "orcamento previsto"},
            "date": set(),
            "acronym_or_title": {"nome do projeto", "como se chama"},
        }

        def has_any_token(*markers: str) -> bool:
            return any(marker in question_tokens for marker in markers)

        def has_any_phrase(markers: set[str]) -> bool:
            return any(marker in normalized_question for marker in markers)

        if question_tokens & {"cidade", "pais"}:
            return "generic_short_fact"
        if normalized_question and (
            has_any_token("quanto", "valor", "orcamento", "bolsa", "reais", "custo")
            or has_any_phrase(phrase_markers["money"])
        ):
            return "money"
        if normalized_question and has_any_token(
            "quando",
            "data",
            "dia",
            "mes",
            ...
            return "date"
        if normalized_question and (
            has_any_token("sigla", "acronimo", "acronym", "titulo")
            or has_any_phrase(phrase_markers["acronym_or_title"])
        ):
    ...
    def _score_support_span_alignment(
        ...
        *,
        ...
        rare_overlap = sum(
            1 for term in set(query_profile.get("rare_terms") or set()) if term in normalized_span
        )
        role_overlap = sum(
            1
        ...
        aligned = bool(
            ...
            or (answer_shape == "person_or_org" and (company_cue_hits or role_overlap) and exact_overlap >= 1)
    ...
    def ask_question_with_trace(self, question: str, chat_history=None) -> dict[str, object]:
        ...
        if not target_document_name and len(document_shortlist) > 1 and retrieval_intent in {"specific_fact", "entity_lookup"}:
            target_document_name = document_shortlist[0]
            matched_documents = [target_document_name]
            resolver_status = "single_match"
            resolver_selection_mode = resolver_selection_mode or "auto_top_document"
