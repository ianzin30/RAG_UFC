...
# Simple: Combine search results to answer user questions
...
class RetrievalAggregationMixin:
    ...
    def _is_factoid_context_intent(
        self,
        ...
        *,
        ...
        if retrieval_intent in {"specific_fact", "entity_lookup"}:
            return True
        return retrieval_intent == "document_expansion" and bool(target_document_name)

    def _format_focused_evidence_block(
        self,
        ...
        *,
        ...
        if not selected_evidence_spans:
            return ""

        lines = ["Evidencia focalizada"]
        if target_document_name:
            lines.append(f"Documento-alvo: {target_document_name}")
        if resolved_question:
            lines.append(f"Pergunta contextualizada: {resolved_question}")
        lines.append("Trechos mais alinhados:")
        for index, span in enumerate(selected_evidence_spans[:3], start=1):
            document_name = str(span.get("document_name") or "documento").strip()
            section_title = str(span.get("section_title") or "").strip()
            ...
            label += f" ({chunk_kind})"
            lines.append(label)
            lines.append(f"- {str(span.get('text') or '').strip()}")
        return "\n".join(lines).strip()

    def _build_minimal_raw_context(
        self,
        docs,
        *,
        selected_evidence_spans: list[dict[str, object]],
        ...
        preferred_orders = {
            (
                str(span.get("document_name") or "").strip(),
                span.get("chunk_order"),
            )
        ...
        for doc in docs:
            ...
            if preferred_orders and key not in preferred_orders and len(raw_blocks) >= 2:
                continue
            compact = self._strip_retrieval_wrapper(getattr(doc, "page_content", "") or "")
            normalized = self._normalize_identifier(compact)
            if not compact or not normalized or normalized in seen:
                continue
            seen.add(normalized)
            raw_blocks.append(compact)
            if len(raw_blocks) >= 2:
                break
    ...
    def _build_answer_context(
        ...
        *,
        ...
        selected_evidence_spans = list(selected_evidence_spans or [])
        if self._is_factoid_context_intent(
            retrieval_intent,
            target_document_name=target_document_name,
        ):
            focused_evidence = self._format_focused_evidence_block(
                selected_evidence_spans,
                target_document_name=target_document_name,
                resolved_question=resolved_question,
            )
            raw_context = self._build_minimal_raw_context(
                docs,
                selected_evidence_spans=selected_evidence_spans,
            )
            sections = [focused_evidence]
            if raw_context:
                sections.append(f"Contexto bruto minimo:\n{raw_context}")
            if focused_evidence:
