...
# Simple: Parse and process AI agent responses
...
class RAGServicePlanningSupportMixin:
    ...
    def _build_candidate_documents_payload(
        self,
        candidate_names: list[str],
        score_lookup: dict[str, dict[str, object]] | None = None,
        ...
        for document_name in candidate_names:
            ...
            if entry is None:
                continue
            match_payload = dict((score_lookup or {}).get(document_name) or {})
            diagnostics = dict(entry.get("extraction_diagnostics") or {})
            payload.append(
                {
                    ...
                    "high_value_excerpt": entry.get("high_value_excerpt"),
                    "source_quality": diagnostics.get("source_quality"),
                    "resolver_score": match_payload.get("score"),
                    "resolver_reason": match_payload.get("reason"),
                    "matched_terms": list(match_payload.get("matched_terms") or []),
