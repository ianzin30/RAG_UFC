...
# Simple: Generate multiple names documents can be called by

import re
from pathlib import Path

from .AliasDates import DocumentAliasDateMixin
...
class DocumentAliasMixin(DocumentAliasDateMixin):
    ...
    def _is_resolver_generic_query_term(self, term: str) -> bool:
        return term in self._get_generic_query_terms()
    ...
    def _is_discriminative_query_term(self, term: str) -> bool:
        ...
        if re.fullmatch(r"\d{4}", term):
            return False
        return not self._is_resolver_generic_query_term(term)
